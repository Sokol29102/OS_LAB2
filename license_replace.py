...
def c_files(root: Path, recursive: bool) -> Iterable[Path]: #виписує всі .c
    return (p for p in (root.rglob("*.c") if recursive else root.glob("*.c")) if p.is_file()) #генерує нам шляхи до файлів (може рекурсивно)
...
def replace_header(src: str, new_header: str) -> Tuple[str, bool]: #замінюємо коментар у тексті файлу
    ...
    return src[:start] + new_header + src[end:].lstrip("\n"), True #замінюємо старий коментарь на новий і прибираємо зайві порожні рядки після нього
...
def parse_args() -> argparse.Namespace: #розбір аргументів командного рядка
    ...
    p.add_argument("--dir", required=True, type=Path, help="Directory with .c files") #папка з файлами які треба міняти
    p.add_argument("--source-exemplar", required=True, type=Path, #приклад файла з ліцензією яку міняти
                   help="file with old license header")
    p.add_argument("--target-exemplar", required=True, type=Path, #приклад на яку заміняти
                   help="file with new license header")
    p.add_argument("--recursive", action="store_true", #шукати в папках і підпапках рекурсивно короче
                   help="recurse into subdirectories")
    p.add_argument("--threshold", type=float, default=0.75, #наскільки схожими воно буде рахувати їх
...
def main() -> int:
    ...
    tgt_h = extract_header(read(args.target_exemplar)) #тут витягуємо нову (щоб не забути що ці дві строки роблять)
    ...
    for path in c_files(args.dir, args.recursive): #проходимо по всіх файлах в папці( і в папках в папці теж)
        ...
        new_header = format_body(tgt_h.body, fh.style) #формуємо новий коментар з тіла нової ліцензії в ідентичному до оригінальної формі (ну або блок або строки)
